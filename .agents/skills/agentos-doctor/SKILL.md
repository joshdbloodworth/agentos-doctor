---
name: agentos-doctor
description: Audit a configured, file-backed AgentOS or personal operating system without changing it. Use when asked to check operating-file health, source-of-truth drift, dashboard and JSON-state parity, governance gates, receipt recency, or optional native Codex runtime health.
---
...
2. Run:
   ...
   environment makes native Codex checks unreliable.
...
- Keep native `codex doctor` findings separate from operating-system findings.
...
only the files and parity rules that actually exist in the target system. Read
`references/configuration.md` when adding optional governance or receipt
checks.
